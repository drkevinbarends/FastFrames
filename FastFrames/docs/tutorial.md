...
For the cases of per-UniqueSampleProcessing, the user needs to use the `const UniqueSample& id` object that is passed to the relevant methods in the custom class as an argument and do decision based on this object, i.e.:
```c++
if (id.dsid() == XXX) {
  ...
}
