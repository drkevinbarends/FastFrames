...
class VariableWrapper   {
    public:
         ...
         * @param type
         */
        inline void setUnderOverFlowType(const std::string& type) {
            m_variable->setUnderOverFlowType(stringToUnderOverFlowType(type));
        }
         ...
         */
        inline std::string underOverFlowType() const {
            return underOverFlowTypeToString(m_variable->underOverFlowType());
        }
    ...
    private:
        ...
        static std::vector<std::tuple<std::string, UnderOverFlowType> > s_underOverFlowTypeTypes;

        static std::string underOverFlowTypeToString(UnderOverFlowType type) {
            for (const auto& [typeName, typeType] : s_underOverFlowTypeTypes) {
                if (typeType == type) {
                    return typeName;
                }
            }
            throw std::runtime_error("Unknown under/overflow enum type: " + std::to_string(static_cast<int>(type)));
        };

        static UnderOverFlowType stringToUnderOverFlowType(const std::string& type) {
            for (const auto& [typeName, typeType] : s_underOverFlowTypeTypes) {
                if (StringOperations::compare_case_insensitive(typeName, type)) {
                    return typeType;
                }
            }
            throw std::runtime_error("Unknown under/overflow type: " + type);
        };
};
...
std::vector<std::tuple<std::string, UnderOverFlowType> > VariableWrapper::s_underOverFlowTypeTypes = {
    {"none",        UnderOverFlowType::NO_UNDER_OVER_FLOW_MERGE},
    {"underflow",   UnderOverFlowType::MERGE_UNDERFLOW},
    {"overflow",    UnderOverFlowType::MERGE_OVERFLOW},
    {"both",        UnderOverFlowType::MERGE_BOTH},
};
